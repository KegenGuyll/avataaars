...
type WinterHat3Props = {
  children?: React.ReactNode
}

const WinterHat3: ExtendedComponent<WinterHat3Props> = ({
  children,
}: WinterHat3Props) => {
  const id = useId()
  const filter1 = `${id}-filter1`
  const mask1 = `${id}-mask1`
  const mask2 = `${id}-mask2`
  const path1 = `${id}-path1`
  const path2 = `${id}-path2`
  const path3 = `${id}-path3`
