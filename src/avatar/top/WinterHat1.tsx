...
type WinterHat1Props = {
  children?: React.ReactNode
}

const WinterHat1: ExtendedComponent<WinterHat1Props> = ({
  children,
}: WinterHat1Props) => {
  const idPrefix = useId()
  const filter1 = `${idPrefix}-filter`
  const mask1 = `${idPrefix}-mask1`
  const mask2 = `${idPrefix}-mask2`
  const path1 = `${idPrefix}-path1`
  const path2 = `${idPrefix}-path2`
  const path3 = `${idPrefix}-path3`
