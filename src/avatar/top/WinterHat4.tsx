...
interface WinterHat4Props {
  children?: ReactNode
}

const WinterHat4: ExtendedComponent<WinterHat4Props> = ({
  children,
}: WinterHat4Props) => {
  const reactId = useId()
  const filter1 = `${reactId}-filter1`
  const mask1 = `${reactId}-mask1`
  const mask2 = `${reactId}-mask2`
  const mask3 = `${reactId}-mask3`
  const mask4 = `${reactId}-mask4`
  const path1 = `${reactId}-path1`
  const path2 = `${reactId}-path2`
  const path3 = `${reactId}-path3`
  const path4 = `${reactId}-path4`
  const path5 = `${reactId}-path5`
