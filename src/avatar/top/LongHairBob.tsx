...
interface LongHairBobProps {
  children?: React.ReactNode
}

const LongHairBob: ExtendedComponent<LongHairBobProps> = ({
  children,
}: LongHairBobProps) => {
  const reactId = useId()
  const mask1 = `${reactId}-mask-1`
  const mask2 = `${reactId}-mask-2`
  const path1 = `${reactId}-path-1`
  const path2 = `${reactId}-path-2`
